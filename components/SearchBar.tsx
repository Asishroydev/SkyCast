...
export const SearchBar: React.FC<SearchBarProps> = ({ onSearch, isLoading }) => {
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        // Fetch more results (count=10) to allow for re-sorting Indian cities to the top
        ...
        if (data.results && Array.isArray(data.results)) {
          // Sort results to prioritize India
          const sortedResults = data.results.sort((a: any, b: any) => {
            const isIndiaA = (a.country_code === 'IN' || a.country === 'India');
            const isIndiaB = (b.country_code === 'IN' || b.country === 'India');
            
            if (isIndiaA && !isIndiaB) return -1;
            if (!isIndiaA && isIndiaB) return 1;
            return 0; // Maintain original API relevance/population order
          });

          // Take top 5 after sorting
          const top5 = sortedResults.slice(0, 5);

          const formattedSuggestions = top5.map((item: any) => {
